...
contract SupplyChain {
    ...
    modifier sold(uint256 _sku) {
        require(items[_sku].state == State.Sold, "State not in Sold");
        _;
    }
    modifier shipped(uint256 _sku) {
        require(items[_sku].state == State.Shipped, "State not Shipped");
        _;
    }
    modifier received(uint256 _sku) {
        require(items[_sku].state == State.Received, "State not Recieved");
        _;
    }
